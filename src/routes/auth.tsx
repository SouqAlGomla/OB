...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "تسجيل الدخول — إعلانات العبور" },
      { name: "description", content: "سجل دخولك أو أنشئ حساب جديد للنشر على موقع إعلانات مدينة العبور" },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (error) {
      toast.error(error.message === "Invalid login credentials" ? "البريد أو كلمة السر غير صحيحة" : error.message);
      return;
    }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-background to-accent/30 px-4 py-12">
      <div className="w-full max-w-md">
        <Link to="/" className="mb-6 flex items-center justify-center gap-2 text-sm text-muted-foreground hover:text-primary">
        ...
        <Card className="border-border/60 shadow-elegant">
          <CardHeader className="text-center">
            <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl gradient-primary shadow-glow">
              <span className="text-2xl font-bold text-primary-foreground">ع</span>
            </div>
            <CardTitle className="text-2xl">أهلاً بك</CardTitle>
            <CardDescription>سجل دخولك أو أنشئ حساب جديد لتبدأ النشر</CardDescription>
          </CardHeader>
          <CardContent>
            <Button
              type="button"
            ...
            <Tabs value={tab} onValueChange={(v) => setTab(v as "login" | "signup")}>
              ...
              <TabsContent value="login">
                <form onSubmit={handleEmailLogin} className="mt-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="login-pw">كلمة السر</Label>
                    <Input
                      id="login-pw"
                      type="password"
                      value={password}
              ...
              <TabsContent value="signup">
                <form onSubmit={handleSignup} className="mt-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="signup-pw">كلمة السر</Label>
                    <Input
                      id="signup-pw"
                      type="password"
                      value={password}
